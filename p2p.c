...
#define POEM_LINE_CLIENT 12
#define POEM_LINE_NUM 12
...
void *client(void *arg) {
    ...
    while(is_poem_ready() < 0) {
        ...
        if (client_socket < 0) printf("Client ERROR opening socket\n");
            ...
            if (numbers[poem_line_number] == poem_line_number) {

                status = write(client_socket, no, 256);
                printf("Client sent message %s\n", no);

            } else if (poem_line_number > 0 && poem_line_number < 12) {
                ...
                numbers[poem_line_number] = poem_line_number;
...
void *server(void *arg) {
    ...
    int32_t poem_line_number = POEM_LINE_NUM;
    int32_t poem_line_length = POEM_LINE_LENGHT;
    int status;
